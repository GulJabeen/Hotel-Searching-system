...
<blockquote>
  <?php
   ...
   error_reporting(0);
  $Searchall=$_POST['buttonsearchall'];
  $CITYsearch=$_POST['city'];
  $Byaverage=$_POST['classcheck'];
  $byrating=$_POST['classrating'];
  echo $Searchall;
  if($Searchall=="SearchALL")
 {$sqlselect="SELECT * FROM  `hotel_details`";}
else if($CITYsearch!=null && $Byaverage=="dont select" && $byrating=="dont select")
{$sqlselect="SELECT * FROM  `hotel_details` WHERE hotel_city='$CITYsearch'";}
else if( $CITYsearch=="dont select" && $Byaverage!="dont select" && $byrating=="dont select")
{$sqlselect="SELECT * FROM  `hotel_details` WHERE hotel_class='$Byaverage'";}
else if($CITYsearch=="dont select" && $Byaverage=="dont select"&& $byrating!="dont select")
{$sqlselect="SELECT * FROM  `hotel_details` WHERE rating='$byrating'";}
else if($CITYsearch!="dont select" && $Byaverage!="dont select" && $byrating=="dont select")
{$sqlselect="SELECT * FROM  `hotel_details`  WHERE hotel_class='$Byaverage' 
AND   hotel_city='$CITYsearch'  ";}
else if($CITYsearch!="dont select" && $Byaverage=="dont select" && $byrating!="dont select")
{$sqlselect="SELECT * FROM  `hotel_details`  WHERE rating='$byrating' 
AND   hotel_city='$CITYsearch'  ";}
else if($CITYsearch=="dont select" && $Byaverage!="dont select" && $byrating!="dont select")
{$sqlselect="SELECT * FROM  `hotel_details`  WHERE rating='$byrating' 
AND   hotel_class='$Byaverage'   ";}
else if($CITYsearch!="dont select" && $Byaverage!="dont select" && $byrating!="dont select")
{$sqlselect="SELECT * FROM  `hotel_details`  WHERE hotel_class='$Byaverage' 
AND   hotel_city='$CITYsearch' AND rating='$byrating' ";}
...
?>
   ...
   <tr>
     <td colspan="6"><form id="form1" name="form1" method="post"    action="Login.php">
